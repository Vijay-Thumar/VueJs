  ...
  actions: {
    ...
    async signupUser({ commit }, payload) {
      ...
      const response = await axios
        .post(`${process.env.VUE_APP_AUTH_TIO_API}`, payload)
        .then((response) => {
    ...
    async signUpAuth({ commit }, payload) {
      ...
      const responce = await axios
        .post(
          `${process.env.VUE_APP_AUTH_FIREBASE_SIGNUP_API}`,
          postData
        )
        .then((res) => {
    ...
    async loginUser({ commit }, payload) {
      commit(`form/setLoading`, true, { root: true });
      const response = await axios
        .get(`${process.env.VUE_APP_AUTH_TIO_API}`)
        .then((res) => {
    ...
    async callAuth({ commit }, payload) {
      ...
      const responce = await axios
        .post(
          `${process.env.VUE_APP_AUTH_FIREBASE_LOGIN_API}`,
          postData
        )
        .then((res) => {
          if (res.status === 200) {
