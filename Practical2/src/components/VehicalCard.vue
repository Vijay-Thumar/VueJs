<template>
  <div class="List-home">
    <div class="main_container">
      ...
      <button v-if="vehicalData.price" @click="infoClickHandler(vehicalData.price, vehicalData.heading)" class="info_btn">
        info
      </button>

      <button v-else disabled class="disable_button">Available soon!</button>

      <button type="button" data-toggle="modal" data-target="#editModal" class="edit_button" @click="editClickHandler">
        Edit
      </button>
...
<script>
  ...
  export default {
    props: {
      vehicalData: Object,
      formHandeler: Boolean,
    },
    data() {
      return {
        showForm: this.formHandeler
      }
    },
    methods: {
      infoClickHandler(price, heading) {
