<template>
	<div>
		<span class="add_car">
			<button class="btn btn-primary rounded-pill" @click="
		...
		<CarForm :formHeading="formHeading" :initialValues="initialValues" :formSubmitHandler="formSubmitHandler"
			:formVisibility="formVisibility" :formVisibilityHandler="formVisibilityHandler"
		...
		<div class="flex_container">
				<transition-group appear tag="" @before-enter="beforeEnter" @enter="enter">
					<div v-show="!apiError" v-for="(car, index) in carsDetails" :key="car.id" :data-index="index">
						<VehicleCard :formHeadingChange="formHeadingHandler" :vehicleData="car"
							:editCarHandler="editCarHandler" @deleteIteam="deleteClickHandler"
							:formVisibilityHandler="formVisibilityHandler" :formActionHandler="formActionHandler" />
					</div>
				</transition-group>
...
const CarForm = defineAsyncComponent(() =>
	import( /* webpackChunkName: "addEditCarFormComponent" */ "./CarForm.vue")
);
const VehicleCard = defineAsyncComponent(() =>
	import( /* webpackChunkName: "vehicleCardComponent" */ "./VehicleCard.vue")
);
